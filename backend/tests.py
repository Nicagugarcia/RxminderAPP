...
def test_login_subuser(client):
    r = client.post("/login", json={
        "email": "childA@example.com",
        "password": "pwchild"
    })
    assert_response_ok(r, expected_status=200)
    data = r.json()
    assert data["email"] == "childA@example.com"
    assert data["parent_user_id"] is not None

def test_subuser_inherits_parent_prescriptions(client):
    # parentA is user_id=1, childA is user_id=2 thanks to earlier tests
    # Parent adds a prescription
    pres = {
        "user_id": 1,
        "drug_name": "SharedMed",
        "dosage": "50 mg",
    ...
    r = client.post("/prescriptions", json=pres)
    assert_response_ok(r, expected_status=201)

    # Parent should see 1 med
    rp = client.get("/prescriptions/1")
    assert_response_ok(rp)
    assert len(rp.json()) == 1

    # Subuser should also see the same med
    rc = client.get("/prescriptions/2")
    assert_response_ok(rc)
    data = rc.json()
    assert len(data) == 1
    assert data[0]["medication"]["drug_name"] == "SharedMed"

def test_parent_does_not_see_subuser_meds(client):
    # Subuser adds a med
    pres2 = {
        "user_id": 2,
        "drug_name": "ChildOnlyMed",
        "dosage": "20 mg",
        "frequency": 1,
        "start_date": date.today().isoformat()
    }
    r = client.post("/prescriptions", json=pres2)
    assert_response_ok(r, expected_status=201)

    # Parent list should still have only 1 med
    rp = client.get("/prescriptions/1")
    meds_parent = rp.json()
    assert len(meds_parent) == 1
    assert meds_parent[0]["medication"]["drug_name"] != "ChildOnlyMed"

    # Subuser list should now have 2 meds
    rc = client.get("/prescriptions/2")
    meds_child = rc.json()
    names = {m["medication"]["drug_name"] for m in meds_child}
    assert "SharedMed" in names
    assert "ChildOnlyMed" in names
